...
| 3:30–4:10 | Agentic trace (award path) | `GET /incidents/{id}` JSON | "The machine-readable trace judges the award on: cue → chosen rung → tool result → jump reason. Vision output changed every subsequent tool call in both paths — that's the Agentic Vision bar." |
| 4:10–4:45 | AWS + responsible ops | architecture diagram / ECS console | "Runs on Fargate/Graviton from ECR; S3 for privacy-filtered clips; EventBridge cue bus. Reserved-phone enforcement in code, quiet hours, failure modes documented honestly — including what this demo is NOT (no medical claims, no real telephony)." |
| 4:45–5:00 | Close | README / repo | "Care Ladder: confirm before you escalate. OpenCV 5 sees, the ladder decides, every step is on the record." Link to repo + live URL. |
